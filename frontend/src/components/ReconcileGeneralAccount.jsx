                ...
                <tbody>
                  ${unclearedTransactions
                    .map((transaction) => {
                      ...
                      // Pattern 1: Look for address after "Paid to: ... - Address" format
                      const paidToAddressMatch = description.match(
                        /Paid to: [^-]+ - (.+)$/
                      );
                      if (paidToAddressMatch) {
                        address = paidToAddressMatch[1].trim();
                      } else {
                        // Pattern 2: Look for address after "Received from: ... - Address" format
                        const receivedFromAddressMatch = description.match(
                          /Received from: [^-]+ - (.+)$/
                        );
                        if (receivedFromAddressMatch) {
                          address = receivedFromAddressMatch[1].trim();
                        } else {
                          // Pattern 3: Look for address after "Trade #: XXX - Name - Address" format
                          ...
                          if (tradeAddressMatch) {
                            ...
                            // Pattern 4: Fallback to last part after last dash
                            const addressMatch =
                              description.match(/- ([^-]+)$/);
                            if (addressMatch) {
                      ...
                      // Format payee based on what's actually in the description
                      if (payee !== "-") {
                        // Check if the description contains "Paid to:" or "Received from:"
                        const hasPaidTo = description
                          .toLowerCase()
                          .includes("paid to:");
                        const hasReceivedFrom = description
                          .toLowerCase()
                          .includes("received from:");

                        if (
                          hasPaidTo &&
                          !payee.toLowerCase().includes("paid to:")
                        ) {
                          payee = `Paid to: ${payee}`;
                        } else if (
                          hasReceivedFrom &&
                          !payee.toLowerCase().includes("received from:")
                        ) {
                          payee = `Received from: ${payee}`;
                        } else if (!hasPaidTo && !hasReceivedFrom) {
                          // Fallback: use transaction type if no clear indicator in description
