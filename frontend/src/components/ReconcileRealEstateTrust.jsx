                ...
                <tbody>
                  ${unclearedTransactions
                    .map((transaction) => {
                      ...
                      // Pattern 1: Look for address after "Ref: ... - Address" format
                      const refAddressMatch =
                        description.match(/Ref: [^,]+ - (.+)$/);
                      if (refAddressMatch) {
                        address = refAddressMatch[1].trim();
                      } else {
                        // Pattern 2: Look for address after "Paid to: ... - Address" format
                        ...
                        if (paidToAddressMatch) {
                          ...
                          // Pattern 3: Look for address after "Received from: ... - Address" format
                          ...
                          if (receivedFromAddressMatch) {
                            ...
                            // Pattern 4: Look for address after "Trade #: XXX - Name - Address" format
                            ...
                            if (tradeAddressMatch) {
                              ...
                              // Pattern 5: Look for address after "EFT#... - ... - Address" format
                              const eftAddressMatch = description.match(
                                /EFT#[^-]+ - [^-]+ - (.+)$/
                              );
                              if (eftAddressMatch) {
                                address = eftAddressMatch[1].trim();
                              } else {
                                // Pattern 6: Fallback to last part after last dash
                                const addressMatch =
                                  description.match(/- ([^-]+)$/);
                                if (addressMatch) {
                      ...
                      if (payee === "-") {
                        ...
                        const receivedFromMatch = description.match(
                          /Received from: ([^,]+?)(?:\s*,\s*Ref|$)/
                        );
                        if (receivedFromMatch) {
                          payee = receivedFromMatch[1].trim();
                        } else {
                          // Fallback to "Paid to" for payments
                          const paidToMatch = description.match(
                            /Paid to: ([^-]+?)(?:\s*-\s*Commission Trust)?$/
                          );
                          if (paidToMatch) {
